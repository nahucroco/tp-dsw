...
export async function validateUniqueEmail(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!input?.emailAddress) {
      return res.status(400).json({ message: 'emailAddress is required', field: 'emailAddress' });
    }
    ...
    if (id) {
      ...
      if (!current) return res.status(404).json({ message: 'Person not found' });

      const currentEmail = (current.emailAddress ?? '').trim().toLowerCase();
      if (currentEmail === email) {
        req.body.sanitizedInput = { ...(req.body.sanitizedInput ?? input), emailAddress: email, id };
        return next();
      }
    }
    ...
    if (exists && (!id || exists.id !== id)) {
      return res.status(400).json({ message: 'The email already exists', field: 'emailAddress' });
    }

    // 6) Reinyectar normalizado (y id si lo tenemos)
    req.body.sanitizedInput = { ...(req.body.sanitizedInput ?? input), emailAddress: email, ...(id ? { id } : {}) };
    next();
  } catch (e) {
